...
if (object_exists(obj_tube)) {
    obj_tube.hspeed = 0;
}

if (object_exists(obj_point)) {
    obj_point.hspeed = 0;
}

if (object_exists(obj_star) && object_exists(obj_farStar)) {
    obj_star.hspeed = 0;
...
if (object_exists(obj_starSpawner))
    instance_deactivate_object(obj_starSpawner);

if (object_exists(obj_tubeSpawner))
    instance_deactivate_object(obj_tubeSpawner);

if (object_exists(obj_drawScore))
    instance_deactivate_object(obj_drawScore);

if (object_exists(obj_hatSpawner))
